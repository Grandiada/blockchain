...
async function main() {
  ...
  console.log(
    "Deploying SoulboundVisitCardERC721 with account:",
    deployer.address
  );
  ...
  try {
    const deployTx = await SoulboundVisitCardERC721.getDeployTransaction(
      deployer.address
    );
    ...
    console.warn(
      "  If deployment fails due to insufficient funds, please fund your account.\n"
    );
  }

  // Deploy the contract with deployer as the initial owner
  const soulboundVisitCard = await SoulboundVisitCardERC721.deploy(
    deployer.address
  );
  ...
  const ipfsURI =
    "ipfs://bafkreid3hhj7xomsrxdcnexcqjlbkncig3dr2cd3t3isw2xooi2tomctqa";

  console.log("\nMinting visit card...");
  console.log("IPFS URI:", ipfsURI);

  const tx = await soulboundVisitCard.mintVisitCard(deployer.address, ipfsURI);
  await tx.wait();
